...
- [ ] **Task 1: Create function calling data models** (AC: 1, 2)
  - [ ] Create `src/guarantee_email_agent/llm/function_calling.py`
  - [ ] Implement `FunctionDefinition` model with `to_gemini_tool()` method
  - [ ] Implement `FunctionCall` model for tracking execution
  - [ ] Implement `FunctionCallingResult` model with `email_sent` flag
  - [ ] Write unit tests in `tests/llm/test_function_calling.py`

- [ ] **Task 2: Create FunctionDispatcher** (AC: 3)
  - [ ] Create `src/guarantee_email_agent/llm/function_dispatcher.py`
  - [ ] Implement `execute(function_name, arguments)` method
  - [ ] Map to existing clients: `warranty_client`, `ticketing_client`, `gmail_client`
  - [ ] Add structured logging with `extra={"function": name, "arguments": args}`
  - [ ] Handle unknown function with `ValueError`
  - [ ] Write unit tests in `tests/llm/test_function_dispatcher.py`

- [ ] **Task 3: Add function calling to GeminiProvider** (AC: 2)
  - [ ] Add `create_message_with_functions()` method to `GeminiProvider`
  - [ ] Use `genai.GenerativeModel` with `tools` parameter
  - [ ] Implement multi-turn conversation loop via `model.start_chat()`
  - [ ] Handle `function_call` in response parts
  - [ ] Send `FunctionResponse` back to model after execution
  - [ ] Track all function calls in `List[FunctionCall]`
  - [ ] Enforce max 10 iterations
  - [ ] Set `email_sent = True` if `send_email` succeeded
  - [ ] Write integration tests

- [ ] **Task 4: Update InstructionLoader for functions** (AC: 1)
  - [ ] Modify `src/guarantee_email_agent/instructions/loader.py`
  - [ ] Parse `available_functions` from YAML frontmatter
  - [ ] Validate function schema (name, description, parameters required)
  - [ ] Add `get_available_functions()` method to instruction model
  - [ ] Write unit tests for function loading

- [ ] **Task 5: Add generate_with_functions to ResponseGenerator** (AC: 4)
  - [ ] Modify `src/guarantee_email_agent/llm/response_generator.py`
  - [ ] Add `generate_with_functions()` method
  - [ ] Build system prompt from scenario instruction
  - [ ] Build user prompt with email content and serial number
  - [ ] Call `provider.create_message_with_functions()`
  - [ ] Return `FunctionCallingResult`

- [ ] **Task 6: Refactor EmailProcessor for function calling** (AC: 4, 7)
  - [ ] Modify `src/guarantee_email_agent/email/processor.py`
  - [ ] Load scenario with `get_available_functions()`
  - [ ] Create `FunctionDispatcher` with existing clients
  - [ ] Call `generate_with_functions()` instead of `generate()`
  - [ ] Validate `result.email_sent == True`
  - [ ] Raise `ProcessingError` with code `email_not_sent` if false
  - [ ] Log function call summary

- [ ] **Task 7: Update scenario instructions** (AC: 1, 6)
  - [ ] Update `instructions/scenarios/valid-warranty.md` - add 3 functions
  - [ ] Update `instructions/scenarios/invalid-warranty.md` - add 2 functions
  - [ ] Update `instructions/scenarios/missing-info.md` - add 1 function
  - [ ] Update `instructions/scenarios/graceful-degradation.md` - add 1 function
  - [ ] Add function usage guidance in `<objective>` section

- [ ] **Task 8: Update eval models for function validation** (AC: 5)
  - [ ] Modify `src/guarantee_email_agent/eval/models.py`
  - [ ] Add `ExpectedFunctionCall` dataclass
  - [ ] Add `ActualFunctionCall` dataclass
  - [ ] Add `expected_function_calls` to `EvalExpectedOutput`
  - [ ] Add `actual_function_calls` to `EvalResult`
  - [ ] Add `mock_function_responses` to `EvalInput`

- [ ] **Task 9: Create eval function validator** (AC: 5)
  - [ ] Create `src/guarantee_email_agent/eval/validator.py`
  - [ ] Implement `validate_function_calls(expected, actual)` function
  - [ ] Check function count matches
  - [ ] Check function names in order
  - [ ] Check `arguments` (exact match) and `arguments_contain` (partial)
  - [ ] Check `result_contains` for result validation
  - [ ] Check `body_contains` for send_email body content
  - [ ] Return list of failure messages
  - [ ] Write unit tests in `tests/eval/test_function_validation.py`

- [ ] **Task 10: Update EvalRunner for function tracking** (AC: 5)
  - [ ] Modify `src/guarantee_email_agent/eval/runner.py`
  - [ ] Create mock dispatcher from `mock_function_responses`
  - [ ] Track actual function calls during execution
  - [ ] Call `validate_function_calls()` after execution
  - [ ] Add failures to result

- [ ] **Task 11: Update EvalReporter for function display** (AC: 5)
  - [ ] Modify `src/guarantee_email_agent/eval/reporter.py`
  - [ ] Show function call trace for each scenario
  - [ ] Format: `1. ✓ check_warranty(serial_number="...") → {result}`
  - [ ] Show ✗ for failed validations with reason

- [ ] **Task 12: Update eval test cases** (AC: 5)
  - [ ] Update `evals/scenarios/valid_warranty_001.yaml` with function expectations
  - [ ] Update other existing eval files
  - [ ] Create additional eval cases covering function scenarios

- [ ] **Task 13: Run full eval suite and verify** (AC: 7)
  - [ ] Run `uv run python -m guarantee_email_agent eval`
  - [ ] Verify pass rate ≥99%
