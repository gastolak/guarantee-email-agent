...
def mock_warranty_client():
    """Create mock warranty client."""
    client = MagicMock()
    client.check_warranty = AsyncMock(return_value={
...
def mock_gmail_client():
    ...
    client = MagicMock()
    client.send_email = AsyncMock(return_value="msg-abc123")
    return client


@pytest.fixture
def dispatcher(mock_warranty_client, mock_ticketing_client, mock_gmail_client):
    """Create dispatcher with all mock clients."""
...
class TestFunctionDispatcherInit:
    ...
    def test_init_with_all_clients(self, mock_warranty_client, mock_ticketing_client, mock_gmail_client):
        ...
        assert dispatcher._warranty_client is mock_warranty_client
        assert dispatcher._ticketing_client is mock_ticketing_client
        assert dispatcher._gmail_client is mock_gmail_client

    def test_init_with_no_clients(self):
        """Test initialization with no clients."""
        dispatcher = FunctionDispatcher()

        assert dispatcher._warranty_client is None
        assert dispatcher._ticketing_client is None
        assert dispatcher._gmail_client is None

    def test_init_with_partial_clients(self, mock_gmail_client):
        """Test initialization with only some clients."""
        dispatcher = FunctionDispatcher(gmail_client=mock_gmail_client)
...
class TestCheckWarrantyExecution:
    ...
    @pytest.mark.asyncio
    async def test_check_warranty_success(self, dispatcher, mock_warranty_client):
        ...
        mock_warranty_client.check_warranty.assert_called_once_with("SN12345")
    ...
    async def test_check_warranty_no_client(self):
        ...
        assert "Warranty client not configured" in result.error_message
    ...
    @pytest.mark.asyncio
    async def test_check_warranty_client_error(self, mock_warranty_client):
        """Test warranty check handles client error."""
        mock_warranty_client.check_warranty = AsyncMock(
            side_effect=ConnectionError("Connection failed")
        )
        dispatcher = FunctionDispatcher(warranty_client=mock_warranty_client)
...
class TestCreateTicketExecution:
    ...
    @pytest.mark.asyncio
    async def test_create_ticket_success(self, dispatcher, mock_ticketing_client):
        ...
        mock_ticketing_client.create_ticket.assert_called_once_with(ticket_args)
    ...
    async def test_create_ticket_no_client(self):
        ...
        assert result.success is False
        assert "Ticketing client not configured" in result.error_message

    @pytest.mark.asyncio
    async def test_create_ticket_client_error(self, mock_ticketing_client):
        """Test ticket creation handles client error."""
        mock_ticketing_client.create_ticket = AsyncMock(
            side_effect=Exception("API error")
        )
        dispatcher = FunctionDispatcher(ticketing_client=mock_ticketing_client)
...
class TestSendEmailExecution:
    ...
    @pytest.mark.asyncio
    async def test_send_email_success(self, dispatcher, mock_gmail_client):
    ...
    @pytest.mark.asyncio
    async def test_send_email_with_thread_id(self, dispatcher, mock_gmail_client):
    ...
    async def test_send_email_no_client(self):
        ...
        assert result.success is False
        assert "Gmail client not configured" in result.error_message
...
class TestGetAvailableFunctions:
    ...
    def test_all_functions_available(self, mock_warranty_client, mock_ticketing_client, mock_gmail_client):
        """Test all functions available when all clients configured."""
        dispatcher = FunctionDispatcher(
            warranty_client=mock_warranty_client,
            ticketing_client=mock_ticketing_client,
            gmail_client=mock_gmail_client
    ...
    def test_partial_functions_available(self, mock_gmail_client):
